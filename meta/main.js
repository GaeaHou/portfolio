...
function renderCommitInfo(data, filteredCommits) {
  ...
  const totalLOC = data.filter(d => filteredCommits.some(c => c.id === d.commit)).length;
  const totalCommits = filteredCommits.length;
  const totalFiles = d3.groups(data.filter(d => filteredCommits.some(c => c.id === d.commit)), d => d.file).length;
  const maxDepth = d3.max(data.filter(d => filteredCommits.some(c => c.id === d.commit)), d => d.depth);
  const longestLine = d3.max(data.filter(d => filteredCommits.some(c => c.id === d.commit)), d => d.length);
  const maxLines = d3.max(
    d3.rollups(data.filter(d => filteredCommits.some(c => c.id === d.commit)), v => d3.max(v, d => d.line), d => d.file),
    d => d[1]
  );
  const stats = [
